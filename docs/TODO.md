...
- [ ] **Evaluar agregar nuevas fuentes de museos para obras verdaderamente icónicas
  ...
  **Ronda de investigación 2026-09-04** (búsqueda web, sin tocar código todavía — ver
  ...
  - ~~**National Gallery of Art (Washington)**~~ — ✅ **integrada el mismo día**
    ...
    `docs/bitacora.md` para el detalle completo. Catálogo: 11.351 → 14.234 obras (solo
    paintings — los prints salían casi sin clasificar, se dejaron afuera).
  - **Getty Museum** — licencia perfecta (CC0 total, ~88.000 imágenes del museo +
    ~78.000 de archivo del Getty Research Institute), pero la API es **JSON-LD /
    "Linked Art" (RDF, con SPARQL)** — mismo patrón de complejidad que tenía la API
    vieja de Rijksmuseum antes de encontrarle el atajo `edm-framed`. No se confirmó
    si existe un atajo equivalente que simplifique a una sola llamada con imagen
    incluida, o si hace falta armar consultas SPARQL / seguir varios saltos de
    enlaces. Investigar eso antes de decidir si conviene integrarlo o posponerlo.
  - **Fitzwilliam Museum** — descartado, ver `CLAUDE.md` (imágenes no-comerciales).
